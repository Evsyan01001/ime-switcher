...
2. 点击菜单栏 ⌨ 图标
3. 按需选择：
   - 「将「当前应用」设为」→ 应用级规则，整个 App 固定用某输入法
   - 「将当前网站（域名）设为」→ 网站级窗口规则，该域名下所有页面生效（浏览器前台时显示）
   - 「将当前页面设为」→ 精确 URL 窗口规则，只匹配当前这一个页面（浏览器前台时显示）
4. 程序自动将规则写入配置文件并立即生效

> 菜单设置的页面规则会插入 `windowRules` 最前（优先级最高），同 pattern 的旧规则自动替换。
